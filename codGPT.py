...
for _ in range(num_hands):
    ...
    data.append([position, card1[0], card1[1], card2[0], card2[1], hand_type, action])

# Convertimos a DataFrame
df = pd.DataFrame(data, columns=["Position", "Card1", "Suit1", "Card2", "Suit2", "Hand Type", "Action"])

# Preprocesamiento: Convertir variables categóricas en números
label_encoders = {}
for col in ["Position", "Card1", "Suit1", "Card2", "Suit2", "Hand Type", "Action"]:
    le = LabelEncoder()
...
def recommend_play(position, card1, suit1, card2, suit2):
    ...
    suit1_encoded = label_encoders["Suit1"].transform([suit1])[0]
    card2_encoded = label_encoders["Card2"].transform([card2])[0]
    suit2_encoded = label_encoders["Suit2"].transform([suit2])[0]
    hand_type = classify_hand((card1, suit1), (card2, suit2))
    hand_type_encoded = label_encoders["Hand Type"].transform([hand_type])[0]
    
    # Predecir acción
    input_data = [[position_encoded, card1_encoded, suit1_encoded, card2_encoded, suit2_encoded, hand_type_encoded]]
...
if __name__ == "__main__":
    ...
    suit1 = input("Ingresa el palo de la primera carta (hearts, diamonds, clubs, spades): ")
    card2 = input("Ingresa la segunda carta: ")
    suit2 = input("Ingresa el palo de la segunda carta (hearts, diamonds, clubs, spades): ")
    
    recommended_action = recommend_play(position, card1, suit1, card2, suit2)
